...
## alcuni comandi utili in un file Rmd 
# \( \) # matematica in linea (via latex) 
# \[ \] # nuova equazione (via latex) 
# `r ` codice valutato da R 

## posizione della working directory 
getwd()
setwd("my/long/path")

# alcune funzioni base - summary statistics 
# sum(); mean(); sd(); var(); cor(); cov(); length() 

# alcune funzioni utili per le matrici 
# cbind(); rbind() # bind columns or rows
# diag() # estrae/fissa la diagonale 
# t() # trasposta 
# solve() # inversione di una matrice (o soluzione di un sistema) 

# le distribuzioni di probabilità in R 
# (ad es normale) 
qnorm(c(0.5,0.975)) # quantili data una probabilità di non-eccedenza x:P(X \leq x) = p
pnorm(c(0, 1.96))   # funzione di ripartizione p:P(X \leq x) = p
# per la T si usa pt()
# per esempio, il p-value di un t.test a due code 
# con tobs = -1.4 e 15 gradi di libertà
2*(1-pt(abs(-1.44), df = 15))


##lm - modelli lineari  -------
df <- data.frame(x1 = runif(15), x2 = runif(15), y  = rnorm(15))
# head(df); summary(df); names(df) 
# plot(df$x1, df[,"x2"])
# points(df$x1, df[,"x2"], col = 2, pch = 16)
...
vcov(fit) ##matrice di covarianza per i parametri stimati 
cov2cor(vcov(fit)) # da cui si può estrarre la matrice di correlazione 
