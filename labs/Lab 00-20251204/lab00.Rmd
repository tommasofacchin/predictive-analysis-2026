...
Possiamo calcolare (e visualizzare in un grafico) alcune caratteristiche della variabile: 
...
abline(v=quantile(mrents$rent,.975), col = "grey", lty =2)
abline(v=quantile(mrents$rent,.025), col = "grey", lty =2)
...
Si possono anche calcolare indici udili ad indagare la _relazione_ tra le due variabili: 

```{r}
cov(mrents$rent, mrents$area)
cor(mrents$rent, mrents$area)
# correlation is the standadized covariance 
cov(mrents$rent, mrents$area)/(sd(mrents$rent) * sd(mrents$area))
n <- nrow(mrents)
(sum((mrents$rent-mean(mrents$rent)) * (mrents$area - mean(mrents$area)))/(n-1))/ (sqrt(sum((mrents$rent-mean(mrents$rent))^2)/(n-1))*sqrt(sum((mrents$area-mean(mrents$area))^2)/(n-1)))
```
...
Desideriamo individuare il valori ($b_0$,$b_1$) che minimizzano SumSq($b_0$,$b_1$). Potremmo usare un ottimizzatone numerico

```{r}
## one can find the b0,b1 values which minimise the sum of squares
## using the optim function
optim(par = c(-4,-18),# initial values
   fn = computeSumSquares, # function to be minimised 
   y = cars$dist, x = cars$speed, method = "BFGS")
# this does a numerical minimisation
# a bit of an overkill here
```

ma in questo caso è possible trovare una soluzione analitica al problema che risulta essere: 
