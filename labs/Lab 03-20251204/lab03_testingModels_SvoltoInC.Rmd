...
```{r}
autompg <- read.csv(file = "labs/data/autompg.csv")
```
...
sum(fit_base$residuals^2)/nrow(autompg)
sum(fit_all$residuals^2)/nrow(autompg)
...
predict(fit_base, newdata = nd, interval = "confidence")
predict(fit_all, newdata = nd, interval = "confidence")
...
### their difference 
(sum(fit_base$residuals^2) - sum(fit_all$residuals^2)  )
# equivalent to 
#
```
...
```{r}
num <- (sum(fit_base$residuals^2) - sum(fit_all$residuals^2))/(fit_base$df.residual - fit_all$df.residual)
...
```{r}
## p-value
1-pf(Fstat, df1 = (fit_base$df.residual - fit_all$df.residual), 
...
```{r}
curve(df(x, 
         df1 = (fit_base$df.residual - fit_all$df.residual), 
         df2 = fit_all$df.residual),
      from=0,to=5, ylab="density", lwd = 1.5)
points(Fstat,0, col = "orange2", pch = 19, cex = 2)
qf(p = .95,df1 = (fit_base$df.residual - fit_all$df.residual), 
         df2 = fit_all$df.residual )
```
...
```{r}
fit_base
summary(lm(mpg ~ wt + year+hp, data = autompg))
anova(fit_base, lm(mpg ~ wt + year+hp, data = autompg))
```
...
set.seed(134); 
fit_useless <- lm(autompg$mpg~cbind(autompg$wt,autompg$year,matrix(rnorm(390*385),ncol=385)))
...
sum(dnorm(autompg$mpg, fit_all$fitted.values, summary(fit_all)$sigma,log = TRUE))
#sum(dnorm(autompg$mpg, autompg$mpg, summary(fit_all)$sigma,log = TRUE))
  #Assume that Y N(mu_hat, s_e)
```
...
```{r}
## AIC 
-2*logLik(fit_base) + 2 * (1+ length(coef(fit_base)))
-2*logLik(fit_all) + 2 * (1+ length(coef(fit_all)))
### one can fit many models and then compare them
AIC(fit_base, fit_all)
```
...
## BIC
n <- nrow(autompg)
-2*logLik(fit_base) + log(n) * (1+ length(coef(fit_base)))
-2*logLik(fit_all) + log(n) * (1+ length(coef(fit_all)))
### one can fit many models and then compare them
BIC(fit_base, fit_all)
AIC(fit_base, fit_all, k = log(n))
```
