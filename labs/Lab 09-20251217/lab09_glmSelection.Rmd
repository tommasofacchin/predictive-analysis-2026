...
chd_mod_selected <- step(chd_mod_additive, trace = 1, k = 2, direction = "backward")
...
La devianza di questo modello selezionato automaticamente differisce dalla devianza spiegata dal modello più complesso? 
...
lines(nd$ldl, 
      ...
      col = "salmon", lty = 3, lwd = 1.4)
lines(nd$ldl, 
      predict(chd_mod_selected, newdata = nd, type = "response"), 
      col = "dodgerblue", lty = 2, lwd = 1.4)
title(main = "P(CHD = 1) - famhist= Absent, age, tobacco, typea fixed to median")
legend(1.2,0.8, legend = c("only ldl","multiple predictors"), col = c("salmon","dodgerblue"), lty = 1)
```

Possiamo anche guardare come cambia la stima della relazione tra colesterolo e malattie coronariche fissando alcuni predittori ma facendo variare l'età: 
...
lines(nd$ldl, 
      predict(chd_mod_selected, newdata = nd, type = "response"), 
      col = "orange", lty = 2)
title(main = "P(CHD = 1) - famhist= Absent, tobacco, typea fixed to median, age varies")
legend(1.2,0.8, legend = c("age = 31","age = 45", "age = 55"), col = c("pink","dodgerblue","orange"), lty = 1)
```
...
Per i GLMs non è possibile derivare analiticamente intervalli di predizione, si calcolano solo intervalli (approssimati) di confidenza per la il parametro di centralità. 

Potete pensare ad un modo in cui si potrebbe ottenere una qualche forma di quantificazione dell'incertezza nell'effettivo outcome che potremmo aspettarci di vedere? 
...
che evidenzia una varianza non costante. 

Potremmo quindi pensare di usare una trasformazione di Box-Cox
...
che mostra come la trasformazione $\sqrt{\ }$ possa essere una scelta sensata

```{r}
modt <- lm(sqrt(Species) ~ . , gala) 
summary(modt)
plot(modt, 1)
AIC(modl, modt) # can't use this, we have different response! 
```

Abbiamo stabilizzato la varianza, ma stiamo modellando un oggetto diverso da quello che ci interessa: $\sqrt{Y}$ invece di $Y$.

Forse possiamo migliorare questa stima con la **regression di Poisson**: 

```{r}
modp <- glm(Species ~ .,family=poisson,gala) 
plot(residuals(modp) ~ fitted(modp), xlab=expression(hat(mu)),
     ylab="Deviance residuals", pch = 16) 
```

Il grafico è abbastanza asimmetrico, difficile confrontare le stime dei (molti) valori con $\hat{\mu}$ piccolo. Facciamo un grafico usando la scala del predittore lineare: 

```{r}
plot(residuals(modp) ~ predict(modp,type="link"), 
     xlab=expression(hat(eta)),ylab="Deviance residuals", pch = 16)
```

Nessuna chiara relazione tra predittore lineare e residui. 

Se guardiamo invece i residui "response" 


```{r}
plot(residuals(modp,type="response") ~ predict(modp,type="link"), 
    xlab=expression(hat(eta)),ylab="Response residuals", pch = 16)
```

vediamo che al crescere della stima per il valore atteso cresce anche la variabilità della variabile casuale, come ci si aspetta sotto una Poisson. 
