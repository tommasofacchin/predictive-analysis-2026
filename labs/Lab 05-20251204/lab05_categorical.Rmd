...
Prestige <- read.table("../data/prestige.csv", sep =",", stringsAsFactors = TRUE)
plot(Prestige)
head(Prestige)
```
...
Evidenza piuttosto forte contro $H_0$: escludere questa interazione cambia di molto la capacità del modello di catturare la variabilità nei dati. Dovendo scegliere un modello in cui permettiamo l'interazione della variabile categorica solo con un predittore continuo sembra essere meglio permettere l'interazione con `income`. 
...
```{r}
par(mfrow=c(1,2))
cvec <- fit_intrinc$coefficients # vector of coefs 
